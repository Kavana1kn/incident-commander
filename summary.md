...
- Provider-portable via the `config/llm_config.hocon` fallback chain (demoed on Mistral
  `mistral-medium-2508`; also runs on Gemini/OpenAI/Anthropic); validated with
  neuro-san's own `hocon_validator_cli`.
